...
class HNEPY(torch.nn.Module):
    ...
    def forward(self, pos_edges, neg_edges, d1_fea, d2_fea, d3_fea, A):
        ...
        #save embedding
        # save_pkl("results/" + "init_d1_emb.pkl", d1_eb1)
        # save_pkl("results/" + "init_d2_emb.pkl", d2_eb1)
        # save_pkl("results/" + "init_d3_emb.pkl", d3_eb1)
...
class Get_bilinear_sim(nn.Module):
    ...
    def forward(self, indices, d1_eb,d2_eb,d3_eb):
        ...
        drug = d1_eb[d1_indices-1]
        indi = d2_eb[d2_indices-434-1]
        adr = d3_eb[d3_indices-self.d2-434-1]
        ...
        try:
            ...
            b2 = nn.Parameter(self.b2) 
            b2 = drug @ b2.permute(2,0,1)
            b2 = b2 @ adr.permute(1,0) #(4, 17)
            b2 = torch.triu(b2, diagonal=1) #表示取矩阵的右上三角，不包括斜对角。
            b2 = torch.reshape(torch.sum(b2),[1,1])
            
            B = cat([b2],1)
            B2 =  torch.mean(self.B22(emb_matrix),0)
            sim = self.sim2(torch.tanh(B + B2 + self.b_lin2))
        #
        # sim = tf.matmul(tf.nn.tanh(B + B2 + self.b_lin), self.V)
